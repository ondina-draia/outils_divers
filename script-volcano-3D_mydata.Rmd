...
allcells <- readRDS(filename)
```


```{r}
# sample data
sample_data <- allcells@meta.data
sample_data$ID <- rownames(sample_data)
sample_data$orig.ident <- gsub(" ", "", sample_data$orig.ident)
Idents(object = allcells) <- "orig.ident"
groups <- levels(allcells)[1:3]
sample_data = sample_data[sample_data$orig.ident %in% gsub(" ", "", groups), ]


# expression data (scaled)
exp <- as.matrix(allcells@assays$RNA@scale.data)
...
Pvals <- lapply(contrasts, function(x){
  ...
  out <- FindMarkers(allcells, ident.1 = vars[1], ident.2 = vars[2], min.pct = 0, 
                     logfc.threshold = 0)
